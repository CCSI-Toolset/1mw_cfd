...
! Check subgrid regions
      CALL CHECK_IC_SUBGRID
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv!
!                                                                      !
! Subroutine: CHECK_IC_SUBRGRID                                        !
! Author: W.Lane                                      Date:     Nov-14 !
!                                                                      !
! Purpose:                                                             !
!                                                                      !
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv!

      SUBROUTINE CHECK_IC_SUBGRID
      
! Global Variables:
!---------------------------------------------------------------------//
! Subgrid type for non cyl-subgrid regions
      use run, only: SUBGRID_TYPE
! Flag: IC contains geometric data and/or specified type
      use ic, only: IC_DEFINED
! Flag: IC contains cyl-subgrid region
      use ic, only: ANY_IC_SG, LOC_IC_SG
! Cyl-subgrid region specification
      use ic, only: IC_D_cyl, IC_a_cyl, IC_T_cyl
! Solid-phase physical properties
      use ic, only: IC_EP_G, IC_EP_S, IC_ROP_S
      use ic, only: IC_T_S, IC_U_S, IC_V_S, IC_W_S, IC_X_S
      use physprop, only: MMAX, SMAX, NMAX, NMAX_s, D_p0, RO_s0, MW_s
! Run-paramters for momentum equations
      use run, only: MOMENTUM_X_EQ, MOMENTUM_Y_EQ, MOMENTUM_Z_EQ
      use run, only: SPECIES_EQ
      use run, only: DRAG_TYPE
! Thermal physical properties
      use physprop, only: C_PS0, K_S0
! Reactions parameters
      use rxns, only: SPECIES_S, SPECIES_ALIAS_S
      
! Global Parameters:
!---------------------------------------------------------------------//
! The max number of ICs and solid-phases
      use param, only: DIMENSION_IC, DIMENSION_M
! Parameter constants
      use param1, only: ZERO, HALF, UNDEFINED, UNDEFINED_I
! Constants
...
! Loop/varaible indices
      ...
      DO ICV = 1, DIMENSION_IC
         IF (IC_DEFINED(ICV)) THEN
            IF (IC_D_CYL(ICV) == UNDEFINED .AND. IC_A_CYL(ICV) == UNDEFINED) CYCLE
            IF (IC_D_CYL(ICV) /= UNDEFINED .AND. IC_A_CYL(ICV) /= UNDEFINED) THEN
               IF (IC_D_CYL(ICV) <= ZERO) THEN
                  WRITE(ERR_MSG, 1105) ICV
                  CALL FLUSH_ERR_MSG(ABORT=.TRUE.)
               ENDIF
               IF (IC_A_CYL(ICV) < 1.414*IC_D_CYL(ICV)) THEN
                  WRITE(ERR_MSG, 1110) ICV
                  CALL FLUSH_ERR_MSG(ABORT=.TRUE.)
               ENDIF
               LOC_IC_SG(ICV) = .TRUE.
               ANY_IC_SG = .TRUE.
            ELSEIF (IC_D_CYL(ICV) == UNDEFINED) THEN
               WRITE(ERR_MSG, 1115) ICV, 'IC_D_cyl'
               CALL FLUSH_ERR_MSG(ABORT=.TRUE.)
            ELSE
               WRITE(ERR_MSG, 1115) ICV, 'IC_a_cyl'
               CALL FLUSH_ERR_MSG(ABORT=.TRUE.)
            ENDIF
         ENDIF
      ENDDO
      ...
      IF (ANY_IC_SG) THEN
         IF (DRAG_TYPE /= 'WEN_YU') THEN
            WRITE(ERR_MSG, 1120) ICV, ''
            CALL FLUSH_ERR_MSG(ABORT=.TRUE.)
         ENDIF
      ENDIF

1105 FORMAT('Error 1105: Initial condition subgrid region ',I3, &
        ' is ill-', 'defined.',/' > IC_D_cyl must be greater than zero.', &
        /'Please correct ', 'the mfix.dat file.')

1110 FORMAT('Error 1110: Initial condition subgrid region ',I3, &
        ' is ill-', 'defined.',/' > IC_a_cyl must be greater ', &
        'or equal than 1.414*IC_D_cyl', &
        /'Please correct ', 'the mfix.dat file.')

1115 FORMAT('Error 1115: Initial condition subgrid region ',I3, &
        ' is ill-', 'defined.',/' > ',A,' is not specified.', &
        /'Please correct ', 'the mfix.dat file.')

1120 FORMAT('Error 1120: Subgrid models require the Wen-Yu ', &
        'drag model. Please correct the mfix.dat file')

      IF (ANY_IC_SG) THEN
         IF (NMAX_S(1) == UNDEFINED_I) NMAX(1) = 1
         MOMENTUM_X_EQ(2) = .FALSE. ! do not solve momentum for stationary phase
         MOMENTUM_Y_EQ(2) = .FALSE.
         MOMENTUM_Z_EQ(2) = .FALSE.
         SPECIES_EQ(3) = .FALSE. ! do not solve species for stationary phase
         DIMENSION_M = 2
         MMAX = 2
         SMAX = 2
         NMAX(2) = 1
         ...
         MW_s(2,1) = 60.0
         SPECIES_s(2,1) = 'Fe'
         SPECIES_ALIAS_s(2,1) = 'Fe'
         DO ICV = 1, DIMENSION_IC
            IF (IC_DEFINED(ICV)) THEN
               IF (.NOT. LOC_IC_SG(ICV)) THEN
                  IC_EP_S(ICV,1) = (1.0 - IC_EP_G(ICV))
                  IC_EP_S(ICV,2) = 0.0
                  IC_ROP_S(ICV,1) = IC_EP_S(ICV,1)*RO_S0(1)
                  IC_ROP_S(ICV,2) = 0.0
               ELSE
                  IC_EP_S(ICV,2) = HALF*PI*IC_D_CYL(ICV)*IC_D_CYL(ICV) &
                                      /(IC_A_CYL(ICV)*IC_A_CYL(ICV))
                  IC_EP_S(ICV,1) = (1.0 - IC_EP_S(ICV,2))*(1.0 - IC_EP_G(ICV))
                  IC_ROP_S(ICV,2) = IC_EP_S(ICV,2)
                  IC_ROP_S(ICV,1) = IC_EP_S(ICV,1)*RO_S0(1)
                  IC_EP_G(ICV) = (1.0 - IC_EP_S(ICV,2))*IC_EP_G(ICV)
                  IC_T_S(ICV,2) = 273.0
                  IC_U_S(ICV,2) = 0.0
                  IC_V_S(ICV,2) = 0.0
                  IC_W_S(ICV,2) = 0.0
...
! /Cyl-subgrid models.
      ...
      RETURN
      END SUBROUTINE CHECK_IC_SUBGRID
...
! Cyl-subgrid model. - WAL 10/2014
      use ic, only: ANY_IC_SG, LOC_IC_SG, IC_D_CYL, IC_A_CYL
      use constant, only: PI
