...
!-----------------------------------------------
      ...
      USE quadric
      USE mms
      USE ic, only: ANY_IC_SG, IC_SG_AT, IC_D_CYL, IC_A_CYL
      USE ic, only: V_T, B1, B2, B3, B4, G1, G2
      IMPLICIT NONE
!-----------------------------------------------
! Dummy Arguments
!-----------------------------------------------
...
! error message 
      CHARACTER*80     LINE(2)
! FOR filtered cylinder subgrid models  - Avik Sarkar, Jul 2013
...
! Sub-grid cylinder filtered model - Avik Sarkar, Jul 2013
...
! not partitioned betn gas & solids. Modeled as a source term.  
                  IF (ANY_IC_SG) THEN
                     ...
                     IF (M == 1 .AND. EP_S(IJK,2) > ZERO .AND. FLUID_AT(IJK)) THEN !drag exerted only on the flowing solids phase
                        ...
                        a_cyl_star = IC_a_cyl(IC_SG_AT(IJK))/L_star
...
! Sub-grid cylinder filtered model ENDS - Sarkar
