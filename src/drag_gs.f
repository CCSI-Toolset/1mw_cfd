...
!-----------------------------------------------
      ...
      USE funits
      USE mms
      USE ic, only: ANY_IC_SG  ! WAL 10/2014
      IMPLICIT NONE
!-----------------------------------------------
! Dummy arguments
!-----------------------------------------------
...
! Note the presence of gas volume fraction in ROPG
      ...
      IF (EP_S(IJK,2) > ZERO) THEN  ! porou media present
         SG_corr= ONE !by default, no subgrid correction
         !subgrid filtered models for horizontal cylinder array 
         ! Avik Sarkar, July 2013
         IF (M == 2) THEN !the cylinder phase porous medium does not affect the gas, 
            !affects only solids; the cyl-suspension drag is implemented as a source term
            SG_corr = ZERO !drag coeff is zero
            !no need to exert any drag force on the gas (mom eq for cylPhaseIndex not solved anyway)
         ELSE !it is the moving solids phase flowing around the cylinders (i.e. cylPhaseIndex), 
            !whose clustering is affected by the cylinders
            solidsRatio = EP_S(IJK,1)/(ONE - EP_S(IJK,2)) !used repeatedly
            !evaluate the h2D function
            ! add by zhijie
            ! IF( phiSolidRatio < 0.0012 ) THEN
            IF (solidsRatio > 0.0D0) then
               ...
               ELSEIF (solidsRatio <= 0.64) THEN !phisSolidRatio above 0.30 to 0.64
