...
! Cyl-subgrid models. - WAL 10/2014

! Initial conditions cyl-subgrid regions

! Flag to identify if cyl-subgrid drag model is used globally
      LOGICAL :: ANY_IC_SG

! Flag to identify if cyl-subgrid model is used locally
      LOGICAL :: LOC_IC_SG(DIMENSION_IC)

! Array to identify what IC region properties to use for subgrid model
      INTEGER, DIMENSION(:), ALLOCATABLE :: IC_SG_AT

! Cylinder diameter in cyl-subgrid model
      DOUBLE PRECISION :: IC_D_cyl(DIMENSION_IC)

! Cylinder spacing in cyl-subgrid model
      DOUBLE PRECISION :: IC_a_cyl(DIMENSION_IC)
      
! Cylinder temperature in cyl-subgrid model
      DOUBLE PRECISION :: IC_T_cyl(DIMENSION_IC)

! Particle terminal velocity in cyl-subgrid model
      DOUBLE PRECISION V_t

! B1 & B2 will have to be  calculated for each cell,
! they are dependent on RO_g, MU_g, etc. See source_v_s.f & source_u_s.f
